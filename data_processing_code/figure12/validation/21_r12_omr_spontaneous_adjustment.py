#!/usr/bin/env python3
"""Test whether r12 OMR structure associations extend beyond spontaneous FCV."""
...
OUTPUT = Path(
    os.environ.get(
        "ZF_OMR_ADJUSTMENT_OUTPUT",
        PACK_ROOT / "statistics" / "robustness" / "omr_spontaneous_adjustment.csv",
    )
)
...
def design_matrix(frame, controls):
    ...
    if "division" in controls:
        dummies = pd.get_dummies(frame["anatomy_group"], drop_first=True, dtype=float)
    ...
    for iteration in range(N_PERMUTATIONS):
        if "division" in controls:
            ...
            for indices in group_indices:
                shuffled[indices] = x_residual[indices][rng.permutation(len(indices))]
        else:
...
def main():
    ...
    analyses = [
        ...
        ("DCApost_OMR_condition_SD", "FU_DCApost", "stim_FCV_condition_sd"),
        ("OO_OMR_condition_SD", "Hard_OO_fraction", "stim_FCV_condition_sd"),

...
| `06_oo_threshold_sensitivity.py` | DCA thresholds, percentile thresholds, near-zero exclusion, soft OO, and source/target composition controls | subject-region OO values across all tested definitions |
| `07_omr_spontaneous_adjustment.py` | OMR associations adjusted for spontaneous FCV and anatomical division | matched spontaneous/stimulus FCV and structural region table |
...
The bundled OO-sensitivity tables are generated by
`data_processing_code/figure12/validation/20_r12_oo_threshold_sensitivity.py`
from the full r12 compact SC and anatomical-unit DCA files. The public
`06_oo_threshold_sensitivity.py` script starts from those compact
subject-region values and recalculates every reported correlation, bootstrap
interval, permutation test, decomposition, and partial correlation. The OMR
control is analysis-level reproducible directly from the bundled matched
region table.

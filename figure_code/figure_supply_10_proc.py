#!/usr/bin/env python3
"""Draw regional zebrafish functional measures from clean Figure 9 data."""
...
FUNCTIONAL_TABLE = (
    ...
    / "derived_data"
    / "figure9"
    / "figure9_recording_region_measures.csv"
)
REGIONS_FILE = (
    PACK_ROOT
    / "derived_data"
...
OUTPUT_PNG = PACK_ROOT / "figures" / "figure_supply_10_proc.png"
OUTPUT_STATS = PACK_ROOT / "statistics" / "figure_supply_10_proc_region_summary.csv"

FEATURES = (
    ("A", "EdgeStdFCV", "FCV"),
    ("B", "FCS", "FCS"),
    ("C", "ProfileCorrDistFCV", "FC reconfiguration degree"),
    ("D", "NetTE", r"$\mathrm{TE}_{\mathrm{net}}$"),
    ("E", "NeighborNetTE", "Neighbor " + r"$\mathrm{TE}_{\mathrm{net}}$"),
)
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    """Load and validate the clean recording-level table and region subset."""
    functional = pd.read_csv(FUNCTIONAL_TABLE)
    regions = pd.read_csv(REGIONS_FILE).sort_values("legacy_order").copy()

    required_functional = {
        "recording_id", "root_area_id", "node", "anatomy_group"
    }
    required_functional.update(metric for _, metric, _ in FEATURES)
    missing = required_functional.difference(functional.columns)
    if missing:
        raise RuntimeError(f"Functional table is missing columns: {sorted(missing)}")
    ...
    data = functional.merge(
        regions[["legacy_order", "root_area_id", "node", "anatomy_group"]],
        on=["root_area_id", "node", "anatomy_group"],
        how="inner",
        validate="many_to_one",
    )
    counts = data.groupby("node")["recording_id"].nunique()
    if len(counts) != len(regions) or not counts.eq(7).all():
        raise RuntimeError("Every canonical region must contain all seven recordings")
    measure_columns = [metric for _, metric, _ in FEATURES]
    if data[measure_columns].isna().any().any():
        raise RuntimeError("Canonical region data contain missing functional measures")
...
def draw_panel(ax, panel_data: pd.DataFrame, ylabel: str) -> None:
    ...
    if np.nanmin(panel_data["mean"].to_numpy(float)) < 0:
...
def main() -> None:
    ...
    fig, axes = plt.subplots(5, 1, figsize=(16.0, 16.0), constrained_layout=False)
    ...
    # Match the published regional-bar display range for the two bounded metrics.
